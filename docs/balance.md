...
| --- | --- | ---: | --- |
| `Store` | Патроны | `10` золота | `+10` патронов |
| `Store` | Лечение | `20` золота | `+20` HP, можно удерживать цифру покупки |
| `Store` | Полное лечение | `500` золота | HP до максимума |
| `Store` | Доп. щит | `100` золота | открывает `100` shield, одноразово |
| `Store` | Восстановление щита | `100` золота | shield до максимума, пункт виден только если щит куплен |
| `HAMMER` | Патроны | `10` золота | `+10` патронов |
| `HAMMER` | Доп. рывок | `100` золота | `+1` max dash charge, одноразово; пункт пропадает после покупки |
| `HAMMER` | Damage upgrade | `100` золота | `x2` урон игрока, одноразово; пункт пропадает после покупки |
| `HAMMER` | Щит | `100` золота | открывает `100` shield, одноразово |
...
| Деньги старт / максимум | `1000 / 999999` |
