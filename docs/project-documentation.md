...
- Shop UI should be authored through `ShopUiPanel` and `ShopItemCard` components for final presentation. For incomplete scenes, `ShopInteractable` falls back to `ShopUiPanel.GetOrCreateDefault()`, which creates a minimal runtime canvas/panel/card grid so `Store` and `HAMMER` still open purchase menus after pressing `E`.
